            ...
            for (var i = 0; i < curBrands.length; i++) {
                ...
                for (var j = 0; j < curSeries.length; j++) {
                    var obj = {
                        ...
                        price: curBrands.eq(i).find('div a.red').eq(j).text()
                    }
                    fetchData[fetchData.length - 1].sub.push(obj);
                    connection.query('insert into car_carlist(carname,price,brandname,state,datatime,time) values(?,?,?,?,?,?)', [
                        obj.name,
                        obj.price,
                        parentobj.name,
                        0,
    ...
    var reptileMove = function(series, callback) {
        ...
        request({
            ...
            curYears.each(function() {
                ...
                var url = 'http://www.autohome.com.cn/ashx/series_allspec.ashx?s=' + s + '&y=' + y;
